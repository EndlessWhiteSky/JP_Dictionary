...
Процедура ПоискВидЧтенияПриИзменении(Элемент)
	...
	Если НЕ ЗначениеЗаполнено(ПоискВидЧтения) Тогда
		Возврат;
	КонецЕсли;
...
Процедура ПолеПоискаПоЧтениюПриИзменении(Элемент)
	ПолеПоиска = "";
	ПолеПоискаПоЧтению = НРег(ПолеПоискаПоЧтению);
	ИнициализацияТаблицыНаФорме();
КонецПроцедуры
...
Функция СтрокаПодходитПодПоиск(СтрокаКанджи)
	...
	Если ЗначениеЗаполнено(ПолеПоиска) Тогда
		...
		РП3 = СтрНайтиПоРегулярномуВыражению(СтрокаКанджи.Представление, ПолеПоиска, , , , Истина);
		Если РП1.НачальнаяПозиция = 0 И РП2.НачальнаяПозиция = 0 И РП3.НачальнаяПозиция = 0 Тогда
