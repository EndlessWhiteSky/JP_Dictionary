...
Процедура ВосстановитьКанджиПоСохраненным(Токены, Сохр_Токены, БекапСостав)
	...
	Для Счетчик = 0 По ДлинаСовпадающейЧасти - 1 Цикл
		...
		Если ЗначениеЗаполнено(ЗаписьБекап.Канджи) Тогда
			...
			КонецКанджи = Счетчик + 1;
			
			Пока Истина Цикл
				
				Если КонецКанджи >= БекапСостав.Количество() Тогда
					КонецКанджи = КонецКанджи - 1;
					Прервать;
				КонецЕсли;
				
				Если БекапСостав[СчетБекап + КонецКанджи].Канджи <> ЗаписьБекап.Канджи Тогда
					КонецКанджи = КонецКанджи - 1;
					Прервать;
				КонецЕсли;
				
				Если БекапСостав[СчетБекап + КонецКанджи].ЭтоУдвоениеКанджи Тогда
					КонецКанджи = КонецКанджи - 1;
					Прервать;
				КонецЕсли;
			...
			// Если нет, то не восстанавливаем, т.к. это может быть изменение канджи
			Если КонецКанджи >= СчетБекап + ДлинаСовпадающейЧасти Тогда
				Продолжить;
			КонецЕсли;
