...
Функция ПодобратьКанджи(Знач Токены) Экспорт
	...
	НастройкиПодсказок = РегистрыСведений.НастройкиПодсказокКанджи.ТекущиеНастройки();
	МаксимумПредложений = НастройкиПодсказок.КоличествоРядовПодсказок * НастройкиПодсказок.КоличествоСтолбцовПодсказок;
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "%МаксимумПредложений", Формат(МаксимумПредложений, "ЧГ=0"));
...
Функция ПотенциальныеКанджи(МассивЦепочекТокенов)
	...
	Для Счет = 0 По ПотенциальныеКанджи.Количество() - 1 Цикл
		ПотенциальныеКанджи[Счет].Номер = Счет;
	КонецЦикла;
...
Функция ТекстЗапросаКанджиПоСпискуНаименований()
	...
	|	ВТ_ПотенциальныеКанджи.Номер КАК Номер
	|ПОМЕСТИТЬ ВТ_Канджи
	|ИЗ
	...
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ ПЕРВЫЕ %МаксимумПредложений
...
// * Номер - Число - номер по порядку сортировки результата
Функция ШаблонТаблицы_ПотенциальныхКанджи()
	...
	Шаблон.Колонки.Добавить("Номер", Новый ОписаниеТипов("Число"));
